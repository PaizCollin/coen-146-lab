...
// Lab 6 - Client
// 11 February 2022
// This C program is for a UDP client that sends a file to a server using packets. This UDP provides reliable transport service utilizing the stop and wait protocol, which includes checksums, sequence numbers. The clientSend fucntion now uses a select() to implement a timer. If the value is zero, there has been no data and the client must retransmit, else there is data and the client then calls recvfrom() to get the ack and process it. 
